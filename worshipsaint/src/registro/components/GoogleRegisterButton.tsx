import type { FC } from 'react';

interface GoogleRegisterButtonProps {
  /** Callback que se ejecutará cuando el usuario haga clic en el botón. */
  onRegister?: () => void;
}

/**
 * Botón visual "Regístrate con Google".
 *
 * Estado actual:
 * - Solo representa la UI.
 * - No ejecuta Firebase ni autenticación real.
 *
 * Próximos pasos:
 * - Conectar Firebase Authentication.
 * - Implementar onRegister con Google Provider.
 */
const GoogleRegisterButton: FC<GoogleRegisterButtonProps> = ({ onRegister }) => {
  const handleClick = () => {
    console.log('[GoogleRegisterButton] Clic en registro con Google.');
    onRegister?.();
  };

  return (
    <button
      type="button"
      onClick={handleClick}
      style={{
        display: 'inline-flex',
        alignItems: 'center',
        justifyContent: 'center',
        gap: '0.75rem',
        width: '100%',
        padding: '0.95rem 1.5rem',
        borderRadius: 'var(--ws-radius-btn)',
        border: '1px solid rgba(44,33,24,0.12)',
        background: '#fff',
        color: '#2c2118',
        fontFamily: 'var(--ws-font)',
        fontWeight: 700,
        fontSize: '1rem',
        cursor: 'pointer',
        boxShadow: 'var(--ws-shadow-card)',
        transition: 'transform 250ms ease, box-shadow 250ms ease'
      }}
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLElement).style.transform = 'translateY(-2px) scale(1.01)';
        (e.currentTarget as HTMLElement).style.boxShadow = '0 18px 40px rgba(44,33,24,0.14)';
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLElement).style.transform = 'translateY(0) scale(1)';
        (e.currentTarget as HTMLElement).style.boxShadow = 'var(--ws-shadow-card)';
      }}
    >
      <svg
        width="20"
        height="20"
        viewBox="0 0 24 24"
        aria-hidden="true"
        style={{ flexShrink: 0 }}
      >
        <path
          d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
          fill="#4285F4"
        />
        <path
          d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          fill="#34A853"
        />
        <path
          d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
          fill="#FBBC05"
        />
        <path
          d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l2.85 2.22c.87-2.6 3.3-4.53 6.16-4.53z"
          fill="#EA4335"
        />
      </svg>
      Regístrate con Google
    </button>
  );
};

export default GoogleRegisterButton;
