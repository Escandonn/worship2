...
Allowed values: `none, default, minimal, low, medium, high, xhigh, max`  
qwen3 models support `none` to disable reasoning and `default` or null to use the model default.  
qwen/qwen3.8-27b additionally supports `low`, `medium`, and `high`. Its default is `none`; `high` selects the model's native `xhigh` mode.  
...
Allowed values: `auto, on_demand, flex, performance, default, null`  
The service tier used for the request. Deployments running in strict OpenAI compatibility report Groq-specific tiers as `default`.
